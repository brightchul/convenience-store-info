...
def get_info_plus_one(item, itemType: ItemType):
...
def get_info_discount(item, itemType: ItemType):
    return {"type": get_item_type_text(itemType),
            "tag_info": get_tag_info(item),
            "img": get_item_img(item),
            "name": get_item_name(item),
...
def get_item_info(item_list, itemType: ItemType, info_function: Callable):
    result_list = []
...
INFO_FUNCTIONS = {
    ItemType.ONE_ONE:  get_info_plus_one,
    ItemType.TWO_ONE:  get_info_plus_one,
    ItemType.GIFT:  get_info_gift,
    ItemType.DISCOUNT:  get_info_discount,
}
...
def get_data_by_type(itemType: ItemType):
    ...
    while (True):
        ...
        result_list.extend(get_item_info(
...
def run():
    ...
    with open("seven11_item.json", 'w', encoding='utf-8') as file:
